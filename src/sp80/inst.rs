...
impl Inst<AssemblyArgs> {
	pub fn assemble<E>(
		...
		match self {
			...
			&Inst::Addw(ref a1, ref a2) => 
				Ok(Inst::Addw(try!(mapper.map_addr_reg(a1)), try!(mapper.map_addr_reg(a2)))),
			...
			&Inst::Subw(ref a1, ref a2) => 
				Ok(Inst::Subw(try!(mapper.map_addr_reg(a1)), try!(mapper.map_addr_reg(a2)))),
